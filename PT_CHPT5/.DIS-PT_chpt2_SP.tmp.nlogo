...
;
;  ;--------Set up destination A------------
;  let destinationpatches patches with [ pxcor >= 25 and pxcor <= 35 and pycor >= 20 and pycor <= 30 ]
;  ask destinationpatches [
;      set pcolor blue
;      set region "destination"
;    ]
;   ask one-of destinationpatches [set plabel "Destination"]
;
;    ;--------Set up home------------
;  let homepatches patches with [ pxcor >= 55 and pxcor <= 60  and pycor >= 20 and pycor <= 30 ]
;  ask homepatches [
;      set pcolor green
;      set region "home"
;    ]
...
to setup
  ...
  ;setup restarant
  let restaurantpatches patches with [pxcor >= 32 and pxcor <= 42 and pycor >= 10 and pycor <= 20]
     ask restaurantpatches [
       set pcolor yellow
       set region "restaurant"
     ]
  ...
  ;--------setup work----------
  let workpatches patches with [pxcor >= 58 and pxcor <= 66 and pycor >= 25 and pycor <= 33]
     ask workpatches [
       set pcolor red
       set region "work"
     ]
...
to go
  ...
  ask turtles [
    ...
    if (breed = citizens) [
      ...
      updateEnviroment
      ifelse(seesCop = true)[
      add-intention "flee" "true"
      ]
      []
      ifelse(atWork = true)[
        add-intention "work" "true"
      ] []
      ifelse(atHome = true)[
      add-intention "move-to-work" "true"
      ] []
      ; code as defined in the include-file "citizens.nls"
      ]
    if (breed = cops) [
      cop_behavior ; code as defined in the include-file "cops.nls"
      ]
  ...
  let nearby-police other cops in-radius citizen-vision
        if any? nearby-police[
         let police min-one-of nearby-police [distance myself]; identify the cop that is nearest
          if police != nobody and patch-here !=[
             set seesCop true
           ]
         ]
